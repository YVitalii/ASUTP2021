/**
 * module
 *  класс симулятора ТРП-08, с логикой
 */
...
var Furnace = require("../furnaceModel/simulator.js");
/**
...
class TRP08 {
   ...
   * @param {number} [options.furnace={}] модель печи
   */
  constructor(options) {
    //#furnace;
    ...
    this.#furnace = new Furnace(options.furnace);

    /**
     * состояние процесса: "startWaiting" / "waiting" / "startHeating" /"heating" / "startHolding" / "holding"
  ...
  /**
   * пуск/стоп регулирования
   * @param {Boolean|Number} 0 - стоп; 1 - пуск
   */
  set mode(mode = 0) {
  ...
  #heat(now) {
    ...
    this.currTime = now;
    this.#furnace.heat(pow);
    this.currT = this.#furnace.getT();
    trace
      ? log(
          "i",
          logN,
  ...
  startHolding() {
    ...
    this.holdingState.end = new Date(
      this.holdingState.start.getTime() + this.Y * 1000
    );
  ...
  holding() {
    ...
    if (this.currT > this.taskT) {
      // текущая температура больше верхней границы регулирования
      this.currTaskT = this.taskT - this.o;
      return;
    }
    if (this.currT < this.taskT - this.o) {
      // текущая температура меньше нижней границы регулирования
      this.currTaskT = this.taskT;
      return;
    }
    return;
