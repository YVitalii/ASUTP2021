...
const driver = new ClassDriverGeneral({
  ...
  comment: {
    ua: "Драйвер терморегулятора ТРП-08",
    en: "Driver of thermoregulator TRP-08",
    ru: "Драйвер терморегулятора ТРП-08",
  },
  timeout: 2000,
});
...
driver.addRegister({
  ...
  get_: function (buf) {
    let res = { err: null, data: { value: fromBCD(buf), note: this.note } };
    return res;
  },
  _set: function (data) {
    return {
...
driver.addRegister({
  ...
  _set: function (data) {
    ...
    let res = {
      err: null,
      data: { data: toBCD(data), FC: 6, addr: this.addr },
    };
    if (res.data.data == null) {
      res.err = new Error(this.ln + "_ERROR::" + res.err.message);
    }
    if (trace) {
      console.log(ln + `res=`);
  ...
  set_: function (buf) {
    ...
    let res = {
      err: null,
      data: { value: buf.readUInt16BE(), note: this.note },
    };
    if (res.data.value == null) {
      res.err = new Error(this.ln + "ERROR::" + res.err.message);
    }
