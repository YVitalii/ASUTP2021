...
class MaxPRO_645 {
  ...
  constructor(iface = null, id = null, props = null) {
    ...
    this.state.AI = {
      ...
      period: 1 * 1000, // на протязі 10 сек після останнього запиту дані рахуються актуальними
    ...
    this.state.DI = {
      ...
      value: 0,
      timestamp: new Date().getTime() - 10 * 60 * 3600 * 1000,
      period: 1 * 1000, // на протязі 1 сек після останнього запиту дані рахуються актуальними
      source: {}, // регістр, отриманий з драйверу, для дод.інформації
    };
    this.state.DO = {
      // дискретний вихід
