...
router.post('/', function(req, res, next) {
  // -- настройки логгера --------------
   let trace=1;
   let logN=logName+"POST:/saveProgramNew => ";
  ...
  try {
    console.log("Спроба зберегти нову програму від користувача:", req.user);
    fs.writeFile(path, newData, err => {
      if (err) console.log(err);
      res.status(200).send("Нова програма " + req.query.id + " успішно збережена в файлі.");
    });
    return
  } catch(err) {
    res.status(500).send("Нову програму " + req.query.id + " не вийшло зберегти в файлі.");
