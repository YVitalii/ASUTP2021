...
let myAddress = null;
setTimeout(() => {
  myAddress = `http://${iface.ipAddr}:${app.get("port")}`;
  console.log(
    gLn +
      `---------------- ${myAddress} server started at ${new Date().toLocaleTimeString()} --------`
  );
  app.set("myAddress", myAddress);
}, 1000);
// let myAddress = `http://${iface.ipAddr}:${app.get("port")}`;
// console.log(
//   gLn +
//     `---------------- ${myAddress} server started at ${new Date().toLocaleTimeString()} --------`
// );
...
app.use(function (req, res, next) {
  console.log(gLn + `noCache headers::myAdress=${app.get("myAddress")}`);
  res.set("Acess-Control-Allow-Origin", app.get("myAddress"));
