  ...
  const text = g.selectAll("text") // выбираем текст
    ...
    .call(text => text // вызываем стрелочную функцию, которая возвращает настроенный текст
      ...
      .join("tspan") // определяем подтекст
        ...
        .text(d => d)
      ); //вставляем текст данных

  const {x, y, width: w, height: h} = text.node().getBBox(); // получаем вычисленные размеры первого элемента text получаем его вычисленные размеры
  text.attr("transform", `translate(${-w / 2},${15 - y})`); // перемещаем нулевую точку в -ширина/2 и на 15 мм ниже
