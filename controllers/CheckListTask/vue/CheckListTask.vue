<template>
    ...
    <ModalWindow v-if="showModal" @close="showModal = false" :show="showModal" :header="task.header"
        :headerButtonExitActive="false">

        <!-- Check List -->
        <CheckList :table="items" @handle-change-status="handleChangeStatus"> </CheckList>
...
const items = ref((() => {
    let trace = 1, ln = gLn + "computed items()::";
    let a = [];
    for (let i = 0; i < props.task.steps.length; i++) {
        const el = props.task.steps[i];
        a.push({ id: props.task.id + "_CheckBoxItem_0" + i, label: el, comment: "", required: true, checked: false });
    }
    if (trace) { console.log(ln + `items=`); console.dir(a); }
    return a;
})());

const handleChangeStatus = (id, state) => {
    let trace = 1, ln = gLn + `handleChangeStatus(${id}, ${state})::`;
    if (trace) { console.log(ln + `Before items.value=`); console.dir(items.value); }
    let item = items.value.find(r => r.id == id);
    if (item) {
        item.checked = state;
    }
    if (trace) { console.log(ln + `After items.value=`); console.dir(items.value); }
};

const disableButtonOk = computed(() => {
    let trace = 1, ln = gLn + `disableButtonOk::`;
    if (trace) { console.log(ln + `items.value=`); console.dir(items.value); }
    if (!items.value || items.value.length == 0) return false;
    return items.value.some(r => r.required && !r.checked);
});

onMounted(() => {
    console.log("UserConfirmTaskShort mounted");
    console.log("state=", props.task.state);
    console.dir(props)
});
